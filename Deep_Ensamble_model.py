...
# Ensemble networks - len of networks results in the number of NNs in the ensemble
...
def network_create(network_name):
    ...
    with tf.variable_scope(network_name):
        ...
        w_fc0 = weight_variable(network_name + '_w_fc0', dense0)
        b_fc0 = bias_variable(network_name + '_b_fc0', [dense0[1]])
    ...
    fc0 = tf.nn.relu(tf.matmul(input_x, w_fc0) + b_fc0)
    fc3 = tf.nn.leaky_relu(tf.matmul(fc0, w_fc3) + b_fc3)
    ...
    loss = tf.reduce_mean(0.5 * tf.log(output_sig_pos) + 0.5 * tf.div(tf.square(y - output_mu), output_sig_pos)) + 1e-06

    # Get trainable variables - for GRAPH see: https://www.tensorflow.org/api_docs/python/tf/Graph#get_collection
    train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, network_name)

    # Gradient clipping for preventing nan -  GradientDescent, Adagrad and Adadelta
...
# Make batch data
def making_batch_groups(data):
    # Making batches(testing) - make batches that are equal to 1 group
    batch_idx = np.random.choice(data.Group.unique())

    data_batch = data[data.Group == batch_idx]

    batch_x = np.array(data_batch.drop(['Discharge_Q', 'SOH_discharge_capacity', 'Group'], axis=1))
    batch_y = np.array(data_batch['Discharge_Q']).reshape(len(data_test), 1)
    test_size = len(data_batch)
    return batch_x, batch_y, test_size
...
for i in range(len(networks)):
    x_input, y, output_mu, output_sig, loss, train_opt, train_vars = network_create(networks[i])
...
train_data_num = len(train_x) #train_x.shape[0]
test_data_num = len(test_x) #test_x.shape[0]
...
#     load calibration data per group
    ...
    #     plt.grid(True)
        ...
        markers, caps, bars = ax1.errorbar(y_true, y_hat, yerr=z_value * sem_calibrated, ecolor='orange', capsize=2,
                                           ...
                                           label='Predicted capacity ($\hat{y}^*$) ±2$\sigma$')
